...
void TablePage::Init(page_id_t page_id, size_t page_size,
  ...
  if (ENABLE_LOGGING) {
    LogRecord rec(txn->GetTransactionId(), txn->GetPrevLSN(), LogRecordType::NEWPAGE, page_id);
    log_manager->AppendLogRecord(rec);
...
bool TablePage::MarkDelete(const RID &rid, Transaction *txn,
  ...
  if (ENABLE_LOGGING) {
    ...
    // add your logging logic here
    Tuple tmp;
    auto ret = GetTuple(rid, tmp, txn, lock_manager);
    assert(ret);

    LogRecord rec(txn->GetTransactionId(), txn->GetPrevLSN(), LogRecordType::MARKDELETE, rid, tmp);
    log_manager->AppendLogRecord(rec);
    txn->SetPrevLSN(rec.GetLSN());
    SetLSN(rec.GetLSN());
  }
...
bool TablePage::UpdateTuple(const Tuple &new_tuple, Tuple &old_tuple,
  ...
  if (ENABLE_LOGGING) {
    ...
    // add your logging logic here
    LogRecord rec(txn->GetTransactionId(), txn->GetPrevLSN(),
...
void TablePage::ApplyDelete(const RID &rid, Transaction *txn,
  ...
  if (ENABLE_LOGGING) {
    ...
    assert(txn->GetExclusiveLockSet()->find(rid) !=
        txn->GetExclusiveLockSet()->end());
    // add your logging logic here
    LogRecord rec(txn->GetTransactionId(), txn->GetPrevLSN(), LogRecordType::APPLYDELETE);
    log_manager->AppendLogRecord(rec);
    txn->SetPrevLSN(rec.GetLSN());
    SetLSN(rec.GetLSN());
  }
...
void TablePage::RollbackDelete(const RID &rid, Transaction *txn,
  ...
  if (ENABLE_LOGGING) {
    ...
    // add your logging logic here
    LogRecord rec(txn->GetTransactionId(), txn->GetPrevLSN(), LogRecordType::ROLLBACKDELETE);
    log_manager->AppendLogRecord(rec);
    txn->SetPrevLSN(rec.GetLSN());
    SetLSN(rec.GetLSN());
  }
